...
parsePassword :: String -> PasswordEntry
parsePassword str =
  PasswordEntry (read $ head xs) (read $ xs !! 1) (head $ xs !! 2) (xs !! 3)
  where
    xs = (filter (/= "") . splitOneOf "-: ") str

instance A.Door Door2 where
  open door = show $ countValid <$> [isValidPart1, isValidPart2]
    where
      countValid :: (PasswordEntry -> Bool) -> Int
      countValid p = (length . filter p) passwords
      passwords :: [PasswordEntry]
      passwords = parsePassword <$> lines (coerce door)
      isValidPart1 :: PasswordEntry -> Bool
      isValidPart1 x = len >= _num1 x && len <= _num2 x
        where
          len = (length . filter (== _char x) . _password) x
      isValidPart2 :: PasswordEntry -> Bool
      isValidPart2 x = result $ (sort . map (== _char x)) chars
        where
          chars = [password !! (_num1 x - 1), password !! (_num2 x - 1)]
